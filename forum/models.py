...
def render_content(content_raw, sender):
    ...
    for user in mentioned_users:
        content_rendered = re.sub(
            ...
            partial(_replace_username, reverse(
                'forum:user_info', kwargs={"pk": user.pk})),
            content_rendered,
            re.M
        )
...
class Thread(models.Model):
    user = models.ForeignKey(USER_MODEL, related_name='threads', verbose_name=_(
        "user"), on_delete=models.CASCADE)
    title = models.CharField(max_length=120, verbose_name=_("title"))
    content_raw = models.TextField(verbose_name=_("raw content"))
    content_rendered = models.TextField(
        default='', blank=True, verbose_name=_("rendered content"))
    ...
    topic = models.ForeignKey('topic', related_name='threads', verbose_name=_(
        "topic"), on_delete=models.CASCADE)
    pub_date = models.DateTimeField(
        auto_now_add=True, db_index=True, verbose_name=_("published time"))
    last_replied = models.DateTimeField(
        auto_now_add=True, db_index=True, verbose_name=_("last replied time"))
    ...
    def increase_view_count(self):
        Thread.objects.filter(pk=self.id).update(
...
class Post(models.Model):
    thread = models.ForeignKey('thread', related_name='replies', verbose_name=_(
        "thread"), on_delete=models.CASCADE)
    user = models.ForeignKey(USER_MODEL, related_name='posts', verbose_name=_(
        "user"), on_delete=models.CASCADE)
    content_raw = models.TextField(verbose_name=_("raw content"))
    content_rendered = models.TextField(
        default='', verbose_name=_("rendered content"))
    pub_date = models.DateTimeField(
        auto_now_add=True, verbose_name=_("published time"))
    hidden = models.BooleanField(default=False, verbose_name=_("hidden"))
...
class Notification(models.Model):
    sender = models.ForeignKey(USER_MODEL, related_name='sent_notifications', verbose_name=_(
        "sender"), on_delete=models.CASCADE)
    to = models.ForeignKey(USER_MODEL, related_name='received_notifications', verbose_name=_(
        "recipient"), on_delete=models.CASCADE)
    thread = models.ForeignKey('thread', null=True, verbose_name=_(
        "thread"), on_delete=models.CASCADE)
    post = models.ForeignKey('Post', null=True, verbose_name=_(
...
class Appendix(models.Model):
    thread = models.ForeignKey('thread', verbose_name=_(
        "thread"), on_delete=models.CASCADE)
    pub_date = models.DateTimeField(
        auto_now_add=True, verbose_name=_("published time"))
    content_raw = models.TextField(verbose_name=_("raw content"))
...
class Topic(models.Model):
    ...
    node_group = models.ForeignKey(NodeGroup, verbose_name=_(
        "nodegroup"), related_name='topics', on_delete=models.CASCADE)
...
class ForumAvatar(models.Model):
    user = models.OneToOneField(
        USER_MODEL, related_name='forum_avatar', on_delete=models.CASCADE)
