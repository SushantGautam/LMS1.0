...
def start(request):
    ...
    course_group = InningGroup.objects.filter(pk__in=sessions.values_list('Course_Group', flat=True))
    courses = CourseInfo.objects.filter(pk__in=course_group.values_list('Course_Code', flat=True),
                                Use_Flag=True)
