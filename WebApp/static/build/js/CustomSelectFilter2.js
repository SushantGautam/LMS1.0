...
(function ($) {
  ...
  window.SelectFilter = {
    init: function (field_id, field_name, is_stacked) {
      ...
      // FIlter selected
      var filterchosen_p = quickElement(
        "p",
        selector_chosen,
        "",
        "id",
      ...
      var filterchosen_input = quickElement(
        "input",
        filterchosen_p,
        "",
        "type",
    ...
    filter_key_press: function (event, field_id, boxtype = "from") {
      ...
      if (
        ...
        if (boxtype == "from")
          SelectBox.move(field_id + "_from", field_id + "_to");
        else SelectBox.move(field_id + "_to", field_id + "_from");
        from.selectedIndex = 0;
    ...
    filter_key_up: function (event, field_id, boxtype = "from") {
      ...
      if (boxtype == "from") {
        ...
        SelectBox.filter(
          field_id + "_" + boxtype,
          document.getElementById(field_id + "_inputchosen").value
        );
      }
